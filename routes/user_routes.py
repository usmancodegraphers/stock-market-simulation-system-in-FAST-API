...
@routes.post("/users/", response_model=UserSchema, status_code=status.HTTP_201_CREATED)
async def create_user(user: UserSchema, db: Session = Depends(get_db)) -> UserSchema:
    ...
    Raises:
        HTTPException: Raised if a user with the same username
        already exists (HTTP 400 Bad Request).
    """
    ...
    new_user = User(
        username=user.username, balance=user.balance, password=hashed_password
    )
...
def get_user(username: str, db: Session = Depends(get_db)) -> GetUser:
    ...
    Raises:
        HTTPException: Raised if the user with the specified
        username is not found (HTTP 404 Not Found).
    """
